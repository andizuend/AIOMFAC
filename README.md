...
- 
### (3a) Optional step for power users
- This subsection describes a few optional installation steps; they can be skipped by most users.
- For applications in which the AIOMFAC model is frequently run, and often with new SMILES of organic compounds as inputs, a faster alternative exists, one that substantially reduces the relatively large overhead in loading times of the Python program for estimation of the glass transition temperature (TgML_SMILES.py). For example, using the speedier option is the case on the Linux server running the [AIOMFAC-web online model(https://aiomfac.lab.mcgill.ca/model.html). The trick is to constantly run the Python program as a separate process in the background, the `TgML_SMILES_watchdog.py` program provided in the TgML_Armeli folder, rather than on demand only. This watchdog Python program imports the large machine learning libraries and third-party dependencies only once during its startup phase. When running, the "watchdog" process monitors the `InputFiles` folder (within the `TgML_Armeli` folder) and processes generated SMILES inputs on the fly. The presence and active running of `TgML_SMILES_watchdog.py` is inquired from within the Fortran subroutine `PureCompViscosity` (from module `ModPureViscosPar`). If the background process is not running, the SMILES are instead processed on demand by the TgML_SMILES.py script -- hence, this step remains optional. 
